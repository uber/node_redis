...
function bufferSliceParts(buffer, maxLength) {
  var result = [];
  var bytesRead = 0;
  ...
  while(true) {
    ...
    var part = buffer.slice(bytesRead, end);
    result.push(part);
    bytesRead += toRead;
  }
}
...
ReplyParser.prototype._parseResult = function (type) {
    ...
    if (type === 43 || type === 45) { // + or -
        ...
        if (this.options.return_buffers) {
            ...
            var slice = this._buffer.slice(start, end);
            var parts = bufferSliceParts(slice, 100 * 1000)
            var strs = parts.map(function (buf) {
